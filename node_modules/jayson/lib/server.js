...
Server.prototype.error = function(code, message, data) {
  if (_.isObject(code)) return code;
  ...
  var error = { code: code, message: message, data: '' };
  if(typeof(data) !== 'undefined' && !_.isEmpty(data)) error.data = data;
  return error;
};
...
Server.prototype.call = function(request, originalCallback) {
  ...
  maybeParse(request, this.options, function(err, request) {
    ...
    var respond = function(error, result, status) {
      if(utils.Request.isNotification(request)) return callback();
      var response = utils.response(error, result, request.id, self.options.version, status);
    ...
    // Authorized the scope
      var scope = Server.Scope[request.params.context.apikey.scope];
      var required = _.isUndefined(method.__scope) ? Server.Scope.app : method.__scope;
      
      if (required > scope) {
	  return respond(self.error({ code: 401, httpStatus: 401, message: 'Insufficient scope.' }));
      }
	...
	// replace the parameter names with the alias
	_.each(request.params, function(param, name) {
	    if (method.__alias && method.__alias[name] !== undefined) {
		request.params[method.__alias[name]] = param;
		delete request.params[name];
	    }
	});

      if (!_.every(request.params, function(param, name) {
        if (_.contains(method.__optional, name)) {
          return true;
        }
        return _.contains(parameters, name);  
      })) {
    ...
    args.push(function(error, result, status) {
      ...
      respond(null, result, status);
    });

      // check for a filter
      if (method.__filter) {

	  var filter = method.__filter;

	  if (_.isString(filter)) {
	      filter = self.options.router.call(self, filter, request.params);
	  }
	  ...
	  args.push(function(error, result) {
	      var _newArgs = _.toArray(arguments);
	      ...
	      _newArgs = _newArgs.slice(2);

	      _.each(_newArgs, function(arg, i) {
		  args[i+1] = arg;
	      });
...
Object.defineProperty(Function.prototype, 'optionalArgs', {
    value: function() {
	this.__optional = this.__optional || [];
	this.__optional = this.__optional.concat(_.toArray(arguments));
	return this;
    }
});

Object.defineProperty(Function.prototype, 'alias', {
    value: function(name, alias) {
	this.__alias = this.__alias || {};
	this.__alias[alias] = name;
	return this;
    }
});

Object.defineProperty(Function.prototype, 'scope', {
    value: function(scope) {
	this.__scope = Server.Scope[scope];
	return this;
    }
});

Object.defineProperty(Function.prototype, 'filterMethod', {
    value: function(method) {
	this.__filter = method;
	return this;
    }
});
